      ...
      <nav className="h-full flex flex-col bg-white border-r shadow-sm px-4 py-0 sidebar">
        ...
        <SidebarContext.Provider value={{ expanded }}>
          <ul
            className={`flex-1 ${
              expanded ? "" : ""
            } sidebar-button space-y-0 sm:space-y-4`}
          >
            <SidebarItem
              to="/dashboard"
            ...
            <SidebarItem
              to="/logout"
              icon={
                <ArrowLeftOnRectangleIcon className="w-5 h-5 sm:w-8 sm:h-8" />
              }
              text="Logout"
  ...
  return (
    <li
      ...
      className={`relative flex items-center py-3 px-5 my-2 font-medium rounded-md cursor-pointer transition-colors group ${
        active ? "bg-black text-white" : "hover:bg-indigo-50 text-gray-600"
      }`}
    >
